...
The system MUST render "O" for open strings and "×" for muted strings at the right edge of the fret space: for `baseFret === 0`, indicators MUST appear at `x = indicatorX(0, 0) - 8 = fretLineX(1) - 20`; for `baseFret > 0`, indicators MUST appear at `x = indicatorX(baseFret, baseFret) - 8 = fretLineX(1) - 20` (both resolve to the same position — the right edge of the first visible fret column). The system MUST use the shape's designated color for visual consistency. The system MUST NOT skip indicator rendering for barre positions.
...
- THEN an "O" appears at the right edge of the fret space for string 1
- AND the "O" is rendered in C shape color (`#2563EB`)
- AND the "O" is positioned at `x = indicatorX(0, 0) - 8 = fretLineX(1) - 20`
...
- WHEN Fretboard renders
- THEN a "×" appears at the right edge of the fret space for string 6
- AND the "×" is rendered in C shape color (`#2563EB`)
- AND the "×" is positioned at `x = indicatorX(0, 0) - 8 = fretLineX(1) - 20`
...
- WHEN Fretboard renders
- THEN a "×" appears at the right edge of the fret space for string 5
- AND the "×" is rendered in G shape color (`#16A34A`)
- AND the "×" is positioned at `x = indicatorX(baseFret, baseFret) - 8 = fretLineX(1) - 20`
...
- WHEN Fretboard renders
- THEN O/× indicators appear at the right edge of the fret space
