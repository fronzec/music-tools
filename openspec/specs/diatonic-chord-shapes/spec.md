...
#### Scenario: Sharp-key voicing uses barre/movable shapes with no open strings

- GIVEN any sharp-spelled key (C#, F#, G#, D#, A#) and any degree
- WHEN `getOpenVoicing` is called
- THEN the returned voicing has no open strings (all played frets are > 0 — barre or movable shape)

> **As-built note:** A# degree I is a valid A-shape barre at fret 1 (`baseFret=1`, barre at fret 1) with no open strings. The invariant that distinguishes sharp-key shapes is the absence of open strings, not `baseFret > 1`. This is what the test suite asserts in `describe('sharp-key no-open-strings ...')`.
...
`slNoteX(0, baseFret)` returns a gutter sentinel value (`SL.LEFT_GUTTER / 2`) for any `baseFret`. Open strings are rendered as O badges in the left gutter, not via the fret window.
...
- THEN it returns a sentinel value strictly less than `slFretLineX(0)` (i.e., `SL.LEFT_GUTTER / 2`), indicating the string renders in the left gutter, not in the fret grid
...
|------|------|-------------|
| `src/lib/theory/openVoicings.ts` | New | `OpenVoicing` type, `OPEN_VOICINGS` data (84 voicings), `voicingRole`, `getOpenVoicing` |
| `src/lib/theory/shapeLayout.ts` | New | `SL` constants + pure geometry functions (`slStringY`, `slFretLineX`, `slNoteX`, `slViewBoxW`, `slViewBoxH`) |
| `src/lib/components/ChordShapeDiagram.svelte` | New | SVG component rendering compact shape diagram with role-colored dots, open/muted gutter badges, barre bar, note-name column |
| `tests/unit/theory/shapeLayout.test.ts` | New | Unit tests for shape geometry module |
| `tests/unit/theory/openVoicings.test.ts` | New | Unit tests for voicing data contract, correctness invariants (84 voicings), and sharp-key no-open-strings assertion |
| `tests/components/ChordShapeDiagram.test.ts` | New | Component tests for ChordShapeDiagram (all spec scenarios) |

## Modified Files

| File | Change |
|------|--------|
| `src/lib/components/DiatonicHarmonizer.svelte` | Additive: import `getOpenVoicing` + `ChordShapeDiagram`; render one diagram per chord card |
