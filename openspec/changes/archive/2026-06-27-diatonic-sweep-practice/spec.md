...
Adds a sweep-practice mode to Progression Builder that animates diatonic arpeggio notes one-by-one on a movable 5th-string-root barre shape over a 24-fret neck. Extends `ChordQuality` to include `'dim'` for complete diatonic progressions (vii°).
...
In sweep mode, each timer tick MUST advance one `ArpeggioNoteEvent` within the active chord. After the final note of the active chord, the next tick MUST advance to event[0] of the next chord. Play/pause and speed controls MUST retain their existing semantics; in sweep mode, speed drives the per-note step interval.

#### Scenario: Step within a chord

- GIVEN sweep mode is active, playback is running, active chord = C major
- WHEN the timer fires N times (N < total events for the chord)
- THEN the fretboard highlights event[N] without advancing to the next chord

#### Scenario: Advance to next chord

- GIVEN the last note of chord[i] just fired
- WHEN the timer fires next
- THEN activeChordIndex becomes i+1 and the highlighted note is event[0] of chord[i+1]

---

### Requirement: Loop Toggle

Progression Builder MUST expose a loop toggle defaulting to OFF. When loop is ON and the last note of the last chord fires, playback MUST restart at event[0] of chord[0]. When loop is OFF, playback MUST stop after the last note.
...
In sweep mode, FullFretboard MUST render with a 24-fret span. In chord/CAGED mode, FullFretboard MUST retain a 14-fret span. The 24-fret span MUST be supplied as a mode-scoped parameter; `FL.MIN_FRET_SPAN` and `FL.MAX_FRET_SPAN` (used by CAGED mode) MUST NOT be modified.
...
## Out-of-Scope Guards (v1)

- The system MUST NOT emit audio signals. `ArpeggioNoteEvent.midi` is precomputed for future use only.
