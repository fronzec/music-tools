...
Define the delta requirements for the Diatonic Harmonizer: a new top-level tool that shows the seven diatonic triads of any major key in degree order, each with its Roman-numeral label, quality, notes, and a full-neck fretboard diagram. This spec covers the new pure theory module (`diatonics.ts`), the new presentational component (`DiatonicHarmonizer.svelte`), and the four-point app-shell registration.

This spec is a DELTA. It does not re-specify any existing tool. All currently passing requirements remain in force.

---

## Section 1 — Pure Theory Module (`src/lib/theory/diatonics.ts`)

### Requirement: MAJOR_SCALE_INTERVALS Constant

The module MUST export a `readonly` constant:
...
interface DiatonicTriad {
  ...
  roman:    'I' | 'ii' | 'iii' | 'IV' | 'V' | 'vi' | 'vii°';  // Roman-numeral label
  ...
  notes:    [string, string, string];      // [root, third, fifth] note names
}
```
...
- THEN it has exactly the fields `degree`, `roman`, `quality`, `rootPc`, `rootName`, `notes`
...
- Reuses `semitoneToNoteName` and `noteNameToSemitone` from `src/lib/theory/notes.ts` for pitch-class / note-name conversions.
- Does NOT import `TRIAD_OFFSETS` from `chords.ts` to classify quality. Quality is derived from the measured semitone gaps (see below).
...
- C major: all 7 triads — full note tuples, `rootPc`, `rootName`, `quality`, `roman`, `degree`.
...
## Section 2 — Presentational Component (`src/lib/components/DiatonicHarmonizer.svelte`)
...
| Notes | `notes` tuple, comma-separated or space-separated | `'D, F, A'` |
...
- `$props()` if the component accepts any props (currently none expected for the top-level wrapper).
...
### Requirement: ViewName Union Updated

`src/lib/types/chord.ts` MUST include `'diatonic-harmonizer'` in the `ViewName` union type.
...
export type ViewName =
  ...
  | ... // existing values
  | 'chord-builder'
  | 'diatonic-harmonizer';  // added
```

#### Scenario: ViewName includes diatonic-harmonizer

- GIVEN `src/lib/types/chord.ts` is imported
- WHEN the `ViewName` type is inspected
- THEN `'diatonic-harmonizer'` is a valid member of the union
- AND `tsc --noEmit` passes without error

### Requirement: VIEW_NAMES Array Updated

`src/lib/routing.ts` MUST include `'diatonic-harmonizer'` in the `VIEW_NAMES` array. The compile-time exhaustiveness guard on this array requires it to stay in sync with the `ViewName` union; `tsc` MUST fail if they diverge.

#### Scenario: VIEW_NAMES includes diatonic-harmonizer

- GIVEN `src/lib/routing.ts` is imported
- WHEN the `VIEW_NAMES` array is inspected
- THEN it contains `'diatonic-harmonizer'`
- AND `tsc --noEmit` passes (exhaustiveness guard satisfied)

#### Scenario: tsc fails if ViewName and VIEW_NAMES diverge

- GIVEN `'diatonic-harmonizer'` is added to `ViewName` but NOT to `VIEW_NAMES`
- WHEN `tsc --noEmit` is executed
- THEN it reports a type error (exhaustiveness guard fires)

### Requirement: Route Branch in App.svelte

`src/App.svelte` MUST add a route branch for `'diatonic-harmonizer'` that renders `<DiatonicHarmonizer>` wrapped in `<svelte:boundary>`, matching the pattern of all other tool route branches.

#### Scenario: Navigating to /diatonic-harmonizer renders the tool

- GIVEN the user navigates to `/diatonic-harmonizer`
- WHEN `App.svelte` resolves the view
- THEN `<DiatonicHarmonizer>` is rendered inside `<svelte:boundary>`
- AND no 404 or blank view is shown

### Requirement: tools.ts Registry Entry

`src/lib/data/tools.ts` MUST add an `active` entry for `'diatonic-harmonizer'` in the **Fretboard & Theory** category alongside Chord Builder.

The entry MUST have `status: 'active'` (not `'coming-soon'` or `'disabled'`).

#### Scenario: Diatonic Harmonizer appears on the home page

- GIVEN the user navigates to `/` (home)
- WHEN the **Fretboard & Theory** category is rendered
- THEN a card or link for Diatonic Harmonizer is visible
- AND clicking it navigates to `/diatonic-harmonizer`

#### Scenario: Tool status is active

- GIVEN `src/lib/data/tools.ts` is imported
- WHEN the `'diatonic-harmonizer'` entry is inspected
- THEN `status` is `'active'`
- AND `view` is `'diatonic-harmonizer'`
...
4. Removing `'diatonic-harmonizer'` from the `ViewName` union in `src/lib/types/chord.ts`
5. Removing `'diatonic-harmonizer'` from `VIEW_NAMES` in `src/lib/routing.ts`
6. Removing the `'diatonic-harmonizer'` route branch from `src/App.svelte`
7. Removing the `'diatonic-harmonizer'` entry from `src/lib/data/tools.ts`
...
## Reference: Modified Files Summary
...
| `src/lib/data/tools.ts` | Add `active` entry in **Fretboard & Theory** category |
